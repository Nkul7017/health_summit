	...
	$.fn.extend({
		autoCompleteMasterAlias:function(_obj)
		{
			...
			*/
			...
			var srchdPool = {};
			var rsltLstBx = '<div class="etb2b-auto-scroll" style="z-index:999;position:relative; clear:both; width:'+slctr.outerWidth()+'px;"><div class="'+slctrId+slctrSufx+'progress" id="'+slctrId+slctrSufx+'progress" style="display:none; position:absolute; right:0; bottom:0;"><span class="sr-only"></span><i style="margin:4px 0;" class="fa fa-spinner fa-pulse fa-2x fa-fw"></i></div><select class="'+slctrId+slctrSufx+' autoSugSelect" id="'+slctrId+slctrSufx+'" size="'+sugViewMaxCnt+'" tabindex="-1" style="position:absolute; width:100%; display:none;"></select></div>';
			if(slctr.next('.etb2b-auto-scroll').length)slctr.next('.etb2b-auto-scroll').remove();
			...
			*/
			...
			function showSearchResult(srchRslt)
			{
				...
				$.each(rsltToShowInSeq, function(key, val){
					optStr += '<option value="'+val["id"]+'">'+val["name"]+'</option>';
				});
